...
OPA2ORS_init            equ		0	; Don't use
						...
						; PWM generator
DAC1CON0_init           equ		(1<<EN)|(B'10'<<PSS0)	; Vref+ <- FVR2
DAC1REF_init			equ		400
DAC2CON0_init           equ		(1<<EN)|(B'10'<<PSS0)	; Vref+ <- FVR2
DAC2REF_init			equ		600
DACLD_init				equ		B'11'

CM1CON0_init			equ		(1<<ON_CM1CON0)|(1<<ZLF)|(1<<HYS)
CM1CON1_init			equ		0	; Don't use
CM1NSEL_init			equ		B'011'	; C1IN3- <- OPA2OUT
CM1PSEL_init			equ		B'1010'	; DAC1
CM2CON0_init			equ		(1<<ON_CM2CON0)|(1<<ZLF)|(1<<HYS)
CM2CON1_init			equ		0	; Don't use
CM2NSEL_init			equ		B'011'	; C2IN3- <- OPA2OUT
CM2PSEL_init			equ		B'1011'	; DAC2
...
COG1RIS0_init           equ		(1<<RIS1)	; C1 Current Lower Limit > Motor Current
COG1RIS1_init           equ		0
COG1RSIM0_init          equ		(0<<RSIM1)	; Rising Edge, add phase delay
COG1RSIM1_init          equ		0
COG1FIS0_init           equ		(1<<FIS2)	; C2 Current Upper Limit > Motor Current
COG1FIS1_init           equ		0
COG1FSIM0_init          equ		(0<<FSIM2)	; Falling Edge, add phase delay
COG1FSIM1_init          equ		0
COG1ASD0_init           equ		(B'11'<<ASDAC0)|(B'10'<<ASDBD0)	; Shutdown, A <- 1, B <- 0
...
PPS_HB_LS_init			equ		B'00101'	; COG1B -> nMOS
						...
						; Brush Detection
T2TMR_init              equ		0
T2PR_init               equ		40			; 10us
T2CON_init              equ		(B'000'<<T2CKPS0)|(B'0000'<<T2OUTPS0)	; Pre 1:1, Post 1:1
T2HLT_init              equ		B'10110'	; One-Shot, Level reset/trigger, High
;T2HLT_init              equ		B'11110'	; One-Shot, Level reset, edge trigger, High
T2CLKCON_init           equ		B'0001'		; Fcy
T2RST_init              equ		B'01010'	; by sync_C1OUT
...
; RA2 output
;PPS_RA2_init			equ		B'11111'				; Rxy source is RA2, 0
;PPS_RA2_init			equ		B'11000'				; Rxy source is CM1
PPS_RA2_init			equ		B'0001'					; Rxy source is CLC1OUT
...
start:	
	...
	if 0
						...
						movlb	11
						; Hysteric Osc. generator
						movlw	DAC1CON0_init
						movwf	s11_DAC1CON0
						movlw	LOW DAC1REF_init
						movwf	s11_DAC1REFL
						movlw	HIGH DAC1REF_init
						movwf	s11_DAC1REFH

						movlw	DAC2CON0_init
						movwf	s11_DAC2CON0
						movlw	LOW DAC2REF_init
						movwf	s11_DAC2REFL
						movlw	HIGH DAC2REF_init
						movwf	s11_DAC2REFH
	...
	if (PPS_RA2_init == B'0001')				; Rxy source is CLC1OUT
...
CLC1POL_init			equ		B'1110'
		...
		;				bcf		s13_COG1ASD0,ASE
						...
						movlb	2
						bsf		CM1CON0,POL_CM1CON0
						bra		$+1
						bcf		CM1CON0,POL_CM1CON0
